...
(function () {
    ...
    function decodeAlphaMap(size) {
        if (decodedMaps.has(size)) return new Float32Array(decodedMaps.get(size));
        const b64 = ALPHA_MAPS_B64[size];
        if (!b64) return null;
        const bin = atob(b64);
        const bytes = new Uint8Array(bin.length);
        for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
        const floats = new Float32Array(bytes.buffer, bytes.byteOffset, size * size);
        decodedMaps.set(size, new Float32Array(floats));
    ...
    // Score how well a corner matches the watermark pattern using NCC-style correlation.
    // The white watermark creates a brightness lift that should correlate with alpha.
    ...
    function processImageBlob(blob) {
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                if (!pos) { URL.revokeObjectURL(img.src); resolve(blob); return; }
    ...
    function normalizeImageUrl(url) {
        if (!isGeminiImageUrl(url)) return url;
        try {
            const parsed = new URL(url);
            const path = parsed.pathname;
            const tail = path.match(/=([^/?#=]+)$/);
            if (tail && /^(?:s|w|h)\d+/i.test(tail[1])) {
                const keepDl = tail[1].endsWith('-d') ? '-d' : '';
                parsed.pathname = path.slice(0, tail.index) + '=s0' + keepDl;
                return parsed.toString();
            }
            if (/=s\d+([^/]*)$/.test(path)) {
                parsed.pathname = path.replace(/=s\d+([^/]*)$/, '=s0$1');
                return parsed.toString();
            }
    ...
    async function processImgElement(img) {
        ...
        const isTarget = isGeminiImageUrl(img.src) ||
    ...
    function scanImages() {
        document.querySelectorAll('img').forEach(img => processImgElement(img));
    }
    ...
    init();
    console.log(`[ClearGem] v${VERSION} loaded`);
})();
